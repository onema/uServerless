...
```scala
class foo extends ApiGatewayHandler with ApiGatewayHandler.Cors {
    def execute(request: AwsProxyRequest, context: Context): AwsProxyResponse = {
...
class EnvFunction extends ApiGatewayHandler with ApiGatewayHandler.Cors with NoopLambdaConfiguration {
